...
function Inscription() {
  ...
  const navigate = useNavigate();
    ...
    async function Load() {
        const result = await axiosClient.get("/inscriptions");
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      async function fetchFormations() {
          try {
              const response = await axiosClient.get('/PresentationFormation');
  ...
  const handleChange = (selectedOption) => {
      ...
      setFormationTariff(selectedOption.label.split('(')[1].replace(' Ar)', '')); // Extraire et stocker le tarif
...
>
    <div className="modal-header">
        <h4>{selectedInscription ? "Modifier l'inscription" : "Ajouter une inscription"}</h4>
    </div>
    <div className="modal-body">
        <form onSubmit={selectedInscription ? update : save}>
    ...
    <div className="payment-options">
        ...
        <div className="form-check">
            ...
            <div className="modal-footer">
                <button 
                    type="submit" 
                    className={selectedInscription ? 'btn btn-primary' : 'btn btn-success'}
                >
                    {selectedInscription ? (
  ...
  <div className="col-12">
    ...
    <div className="card-header pb-0 d-flex align-items-center">
      ...
      >
        ...
        <h2 className="mb-3 ms-3">Liste des étudiants inscrits</h2>
      </div>
      
      <div className="header-bar">
        {/* Bouton pour ajouter une inscription */}
        <button className="btn-create" onClick={handleAddClick}>
          <i className="fas fa-plus"></i> Nouvelle inscription
        </button>
